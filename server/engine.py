...
def weight(a,b):
 ...
 threshold=max(25,float(np.percentile(delta,95))*.5)
 mask=(delta>threshold).astype(np.uint8)
 d=cv2.distanceTransform(mask,cv2.DIST_L2,5);vals=d[d>0]
 return float(np.mean(vals)*2) if len(vals)>8 else None
...
def analyze(design,implementation,config,out,progress,check):
 ...
 for side,a in zip(('design','implementation'),originals):
  ...
  metadata.append({'logicalWidth':lw,'effectiveScale':s,'source':'user_confirmed' if lw or opt.get('effectiveScale') else 'unknown','platform':opt.get('platform','unknown'),'safeTop':opt.get('safeTop',0),'safeBottom':opt.get('safeBottom',0)})
 aa,bb=imgs
 ...
 widths=[config.get(k,{}).get('logicalWidth') for k in ('design','implementation')]
 ...
 if cross: warnings.append('跨宽参考：不对整页几何与换行作精确还原结论，仅比较可匹配的局部外观。')
 ...
 for c in selected:
  ...
  if cross and c in ('component_position','alignment','text_size'):
   cov[c]={'status':'unavailable','reason':'跨宽输入缺少目标宽度设计稿，未对全局布局与字号变化作精确结论。'}
 ...
 for i,j,cost in pairs:
  ...
  dy=yq-yp
  if not cross and (abs(dx)>pos or abs(dy)>pos):
   cat='alignment' if kind=='text' else 'component_position'
   title=('文字边缘偏移' if kind=='text' else '组件位置偏移')
   if bottom:title='底部锚定间距偏差'
   add(cat,title,r,s,[metric('左边缘 x',x,xx),metric('距底部安全区' if bottom else '上边缘 y',yp,yq)],'OCR 边界' if kind=='text' else '轮廓边界','检查对应区域的内边距、约束或相对间距。',conf if sameheight or bottom else 'low')
  ...
  if kind=='text' and not cross and abs(sh/rh-1)>size and abs(sh-rh)>1:
  ...
  if kind=='text' and abs(sh/rh-1)<.3 and abs(sw/rw-1)<.18 and ca is not None and cb is not None and np.linalg.norm(ca-cb)<35:
   wa=weight(aa,r['box']);wb=weight(bb,s['box'])
   if wa and wb and abs(wb/wa-1)>.2*tolerance and abs(wb-wa)>.3:
    add('font_weight','疑似字重'+('偏粗' if wb>wa else '偏细'),r,s,[metric('笔画宽度估计',wa,wb,estimate=True)],'局部二值笔画距离变换','核对字体、字重与抗锯齿；不直接推断 400/600 等属性。','low')
 ...
 stage(4)
 return {'status':'partial' if any(cov[c]['status']!='checked' for c in selected) else 'completed','issues':issues,'coverage':cov,'warnings':warnings,'transforms':maps,'metadata':metadata,'mode':'cross_width_reference' if cross else 'same_width','normalizedSizes':[[a.shape[1],a.shape[0]] for a in imgs],'duration':round(time.monotonic()-start,1),'candidateCounts':[len(r) for r in rs],'matchedCandidates':len(pairs),'unit':unit,'analyzerVersion':'1.0.0'}
