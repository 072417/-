...
ANALYSIS_WORKERS=max(1,min(4,int(os.getenv('ANALYSIS_WORKERS','2'))))
LOCK=threading.RLock(); POOL=ThreadPoolExecutor(max_workers=ANALYSIS_WORKERS); CANCEL={}
...
def health():
 ...
 return {'ok':True,'ocr':HELPER.exists() or bool(shutil.which('tesseract')),'publicMode':PUBLIC,'ai':bool(os.getenv('VISION_API_KEY') and os.getenv('VISION_MODEL') and os.getenv('VISION_BASE_URL')),'provider':urlparse(os.getenv('VISION_BASE_URL','')).hostname,'model':os.getenv('VISION_MODEL',''),'analysisWorkers':ANALYSIS_WORKERS,'widths':[360,375,390,414,430]}
